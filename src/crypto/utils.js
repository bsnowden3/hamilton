 ...
 * @returns {*} signature  - signature as valid hexadecimal string
 */
const sign = (secretKey, message) => {
    const messageHash = sha256(message);
    return schnorr.sign(secretKey, Buffer.from(messageHash, 'hex')).toString('hex');
}

/**
 * Returns true or false whether the produced signature is validly signed publicKey
 * @param {String} pubkey - publicKey
 * @param {String} message 32 byte string hash
